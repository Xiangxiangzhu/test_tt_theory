...
class Trainer(object):
    ...
    def __init__(self, config, agents):
        self.me = None  # to remove static error
        self.config = config
        self.agents = agents
    ...
    def create_agent_to_agent_group_dictionary(self):
        ...
        agent_to_agent_group_dictionary = {
            ...
            "SACTT": "Actor_Critic_Agents",
            "SACTT_wo_at": "Actor_Critic_Agents",
            "SACTT_wo_pf": "Actor_Critic_Agents",
            "SACTT_wo_gt": "Actor_Critic_Agents",
            "SACT_Uni": "Actor_Critic_Agents",
    ...
    def run_games_for_agents(self):
        ...
        if self.config.file_to_save_results_graph:
            if not os.path.exists(self.config.file_to_save_results_graph):
                os.makedirs(self.config.file_to_save_results_graph)
    ...
    def create_object_to_store_results(self):
        ...
        if self.config.overwrite_existing_results_file or not self.config.file_to_save_data_results or not \
                os.path.isfile(self.config.file_to_save_data_results):
            results = {}
        else:
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            print('run number in Trainer.py', run)
            ...
            # print('vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv')
            game_scores, rolling_scores, time_taken = agent.run_n_episodes()
            my_result_record.append(game_scores)
    ...
    def ignore_points_after_game_solved(self, mean_minus_x_std, mean_results, mean_plus_x_std):
        """Removes the data points after the mean result achieves the score required to solve the game"""
    ...
    def visualise_preexisting_results(self, save_image_path=None, data_path=None, colors=None, show_image=True, ax=None,
        ...
        print('gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg')
    ...
    def visualise_set_of_preexisting_results(self, results_data_paths, save_image_path=None, show_image=True,
        ...
        print('mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm')
        if save_image_path:
...
class Tester(object):
    ...
    def run_games_for_agents(self):
        ...
        for agent_number, agent_class in enumerate(self.agents):
            ...
            self.run_games_for_agent(agent_number + 1, agent_class)
            if self.config.visualise_overall_agent_results:
                agent_rolling_score_results = [results[0] for results in self.results[agent_name]]
                print(444444)
                print(agent_rolling_score_results[0])
                plt.plot(agent_rolling_score_results[0])
        plt.show()
        return self.results
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            print("RANDOM SEED ", agent_config.seed)
            # run episodes n times
            game_scores, rolling_scores, time_taken = agent.test_n_episodes()
