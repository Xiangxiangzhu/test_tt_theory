...
class HRL_Higher_Level_DRL_Agent(DDPG):
    ...
    def save_experience(self, memory=None, experience=None):
        ...
        if experience is None:
            experience = self.step_lower_level_states, self.step_lower_level_action_seen, self.reward, \
    ...
    def transform_goal_to_one_most_likely_to_have_induced_actions(self, experience):
        """Transforms the goal in an experience to the goal that would have been most likely to induce the actions
        chosen by the lower level agent in the experience"""
        ...
        goal_candidates = \
            goal_candidate_state_change + goal_candidate_actual_goal + goal_candidate_state_change_random_iterations
        ...
        for goal_ix, goal in enumerate(goal_candidates):
            ...
            for state_ix in range(timesteps_in_experience):
                ...
                if state_ix != timesteps_in_experience - 1:
                    ...
                    goal = EVCSSPHRL.goal_transition(state_obs, goal, next_state)
            if log_probability_total >= max_temp:
                max_temp = log_probability_total
...
class Lower_Level_Agent_Environment_Wrapper(Wrapper):
    ...
    @staticmethod
    def turn_internal_state_to_external_state(internal_state, goal):
    ...
    def update_goal(self, next_state):
        self.meta_agent.goal = EVCSSPHRL.goal_transition(self.meta_agent.higher_level_state, self.meta_agent.goal,
                                                         next_state)
