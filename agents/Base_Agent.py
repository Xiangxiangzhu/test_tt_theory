...
class Base_Agent(object):
    ...
    def __init__(self, config):
        self.memory = None  # to duel with no init problem
        self.agent_name = None  # to duel with no init problem
        # self.exploration_strategy = None  # to duel with no init problem !!can not
        self.me = None  # to duel with static problem
        ...
        # self.action_types = "DISCRETE" if (
        #             self.environment.action_space.dtype == np.int64 or self.environment.action_space.spaces[
        #         0].dtype == np.int64) else "CONTINUOUS"
        self.action_types = "DISCRETE" if (
                self.environment.action_space.dtype == np.int64) else "CONTINUOUS"
        print('self.environment.action_space.dtype', self.environment.action_space.dtype)
        ...
        writer_name = '_EVCSSP_' + config.agent_name + '_' + str(id(self))
        # print('writer_name', writer_name)
        self.writer = SummaryWriter(comment=writer_name, filename_suffix="_tzm_" + config.agent_name)
        print('id(self.writer)', id(self.writer))
    ...
    def get_action_size(self):
        ...
        # print('self.config.__dict__', self.config.__dict__)
        # print('self.environment.__dict__', self.environment.__dict__)
        if "overwrite_action_size" in self.config.__dict__:
            if isinstance(self.config.overwrite_action_size, tuple):
                return self.config.overwrite_action_size[0]
            else:
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        # print('self.game_full_episode_scores', self.game_full_episode_scores)
        # print('self.rolling_results', self.rolling_results)
        # print('time_taken', time_taken)
        return self.game_full_episode_scores, self.rolling_results, time_taken

    def test_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        """Runs game to completion n times and then summarises results and saves model (if asked to)"""
        if num_episodes is None:
            num_episodes = self.config.num_episodes_to_run
        start = time.time()
    ...
    def save_result(self):
        ...
        self.game_full_episode_scores.append(float(self.total_episode_score_so_far))
    ...
    def print_rolling_result(self):
        # print('id_my_writer', id(self.writer))
        print('my_writer', self.game_full_episode_scores[-1], len(self.game_full_episode_scores))
        self.writer.add_scalars('agent_reward', {"top_agent": self.game_full_episode_scores[-1]},
                                len(self.game_full_episode_scores))
        # self.writer.add_scalars('agent_reward', {"top_agent": self.game_full_episode_scores[-1]},
        #                         len(self.game_full_episode_scores))

        """Prints out the latest episode results"""
        text = """"\r Episode {0}, Score: {3: .2f}, Max score seen: {4: .2f}, 
        Rolling score: {1: .2f}, Max rolling score seen: {2: .2f}"""
        sys.stdout.write(
